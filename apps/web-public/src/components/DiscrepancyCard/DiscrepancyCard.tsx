...
const RULE_GUIDES: Record<string, GuideEntry> = {
  'RULE-001': {
    ...
    statute: 'RA 12009 Section 65 permits awards on a single responsive bid under specific conditions. However, recurring single-bidder scenarios are flagged to check for potential tailored specifications.',
    implication: 'When only one supplier participates, there is no price competition, which often leads to higher public procurement costs. It can also point to "tailored specs"—technical requirements drafted specifically to favor one provider.',
  ...
  'RULE-002': {
    ...
    statute: 'RA 12009 Section 39 prohibits splitting government contracts to bypass bidding thresholds, backed by Section 92(d) administrative penalties, except where split lots are justified by region or market capacity.',
    implication: 'Splitting contracts reduces transparency and public oversight by bypassing competitive bidding. Overusing non-competitive methods increases vulnerability to collusion and inflated pricing.',
    action: 'Verify if a legitimate reason existed that justified package/lot splits or alternative procurement. Review the Bids and Awards Committee (BAC) resolution justifying the method.'
  },
  'RULE-003': {
    title: 'Short Posting Window',
    statute: 'RA 12009 Section 50 and GPPB IRR rules govern the required posting windows. Post-Feb 2025 timelines follow the new IRR (up to 45 calendar days for Goods).',
  ...
  'RULE-004': {
    ...
    statute: 'RA 12009 Section 60 mandates that the Approved Budget for the Contract (ABC) is the absolute ceiling for bid prices; bids exceeding this ceiling must be disqualified outright.',
  ...
  'RULE-005': {
    ...
    statute: 'RA 12009 Section 71 leaves variation order caps to the IRR, while Section 89 fixes contract prices, restricting escalations to PSA cost-index spikes exceeding 10% with GPPB pre-approval.',
  ...
  'RULE-006': {
    ...
    statute: 'RA 12009 Section 7 dictates that no procurement shall be undertaken unless it is in accordance with the approved Annual Procurement Plan (APP) of the procuring entity.',
  ...
  'RULE-007': {
    ...
    statute: 'RA 12009 Section 52 requires that eligible bidders must possess the legal, technical, and financial capability matching the specific project scope.',
  ...
  'RULE-008': {
    ...
    statute: 'RA 12009 Section 66 mandates that the Notice to Proceed must be issued to the winning bidder not later than three (3) calendar days from the date of approval of the contract.',
    implication: 'Delays between contract approval and the Notice to Proceed (NTP) can indicate post-facto negotiations or contractor mobilization issues, risking project completion delays.',
  ...
  'RULE-009': {
    ...
    statute: 'RA 12009 Section 58 requires bid opening records to be accessible, reflecting the general transparency principles in Section 3(a). Specific forms are defined under standard GPPB guidelines.',
  ...
  'RULE-010': {
    ...
    statute: 'The 1987 Philippine Constitution Art. IX-D Sec. 2 empowers the Commission on Audit (COA), operationalized under PD 1445 and COA Circulars on disallowances/suspensions.',
  ...
  'RULE-011': {
    ...
    statute: 'Derived sequencing violation from the mandatory stages in RA 12009 Articles VIII-XI (evaluations/post-qualification must happen after bid submission closes).',
  ...
  'RULE-012': {
    ...
    statute: 'RA 12009 Section 100(c), (f), & (i) prohibits bid-rigging/collusive agreements, analyzed using the HHI market concentration convention from the Philippine Competition Act (RA 10667).',
  ...
  'RULE-013': {
    ...
    statute: 'COA Value-for-Money Audits and price reasonableness circulars mandate that government procurement must be economical, ensuring prices are reasonable compared to market standards.',
